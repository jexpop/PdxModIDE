...
**Versión actual**: 1.6.7 (ver `CHANGELOG_ES.md`, `CHANGELOG_EN.md`, `CHANGELOG_CA.md`). 
...
**Modo Cultura** (`CultureLoader`): `LoadCultures` parsea definiciones de `common/culture/cultures/*.txt` y resuelve referencias `color = <nombre>` contra `common/named_colors/*.txt` (raíces juego + mod). El historial de títulos (`history/titles/*.txt`) y de personajes (`history/characters/*.txt`) acepta IDs de personaje numéricos y string (contenido oriental: China/Japón/Corea), por lo que las provincias sin `culture =` directo heredan correctamente la cultura del titular del condado.

### 5.5 `ModuleValidator` (`PdxModIDE.Validation`)

**Diff tres vías no recursivo** (solo archivos de primer nivel): Mod vs Backup, Game vs Backup, Game vs Mod.
...
| `FindDateModules()` | Escanea la raíz del juego recursivamente buscando carpetas no configuradas con archivos de fecha (solo informativo) |
...
- **Etnias en el detalle de la cultura (pestaña Culturas)**: el panel de detalle también muestra las etnias de la cultura (del bloque `ethnicities = { <peso> = <etnia> ... }`, el último bloque de la definición de cultura), cada una en su propia línea como `nombre peso%` (p. ej. `caucasian_blond 25%`). `ExtractEthnicitiesAttribute`/`ParseEthnicityEntries` parsean el bloque ponderado; los pesos se muestran como porcentajes tal como aparecen en el fichero del juego. Los identificadores de etnia no tienen localización en los ficheros del juego, así que se muestran tal cual. Colocado al final del panel de detalle, después de la sección de Gráficos.
...
*Generado: 2026-08-09 | Proyecto: PdxModIDE | Versión: 1.6.7 | Stack: .NET 8 / WPF / SkiaSharp 3.116.1 / System.Text.Json*
