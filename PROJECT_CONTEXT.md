...
**Current version**: 1.8.5 (see the `Changelog/` folder, one file per minor version). Solution: `PdxModIDE.sln` (10 projects — added BookmarksTab + BookmarkLoader; HistoryTab now bookmark-driven date with ViewSelector/ShowNames/ModeToggle repositioned).
...
- `BookmarksTab` (new in 1.8.0): Bookmarks browser with two-section TreeView (Mod + Base game informative). Groups sorted chronologically by `default_start_date` (`_ungrouped` last); mod section shows all mod groups even when empty but hides base-only empty groups; base section shows full base catalogue. Each bookmark shows `start_date`, `group`, `is_playable`, `recommended`, `requires_dlc_flag` and `character = { }` list (with sub-characters). Merged `mod` overrides `base` (duplicate keys hidden). Data from `common/bookmarks/bookmarks/*.txt` + `common/bookmarks/groups/*.txt` via `BookmarkLoader` (read-only MVP, future `ITranslationProvider` for `bookmark_*` loc).
...
*Generated: 2026-09-23 | Project: PdxModIDE | Version: 1.8.5 | Stack: .NET 8 / WPF / SkiaSharp 3.116.1 / System.Text.Json*
